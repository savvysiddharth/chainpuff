...
app.post('/compile', (req, res) => {
  ...
  const solcVersion = soljsonReleases[sourceCode.split('\n')[0]
  .split('^')[1].slice(0, -1)].slice(8,-3);

  solc.loadRemoteVersion(solcVersion, function(err, customSolc) {
    if (err) {
      console.log('error'); // An error was encountered
    } else {
      const preparedInput = prepareCode(sourceCode);
      const output = customSolc.compile(JSON.stringify(preparedInput));
      const parsedOutput = JSON.parse(output);
      const contractName = Object.keys(parsedOutput.contracts.main)[0];
...
function prepareCode(sourceCode) {
  const input = {
    language: 'Solidity',
